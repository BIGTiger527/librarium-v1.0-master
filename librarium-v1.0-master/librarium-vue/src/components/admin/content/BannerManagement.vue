<template>
  <div>
    ...
    <el-link href="/admin/content/guanggaoarticle" :underline="false" target="_blank" class="add-link">
      <el-button type="success" class="el-icon-plus">添加广告</el-button>
    </el-link>
...
<script>
  export default {
    ...
    methods: {
      loadArticles () {
        var _this = this
        this.$axios.get('/guanggaoarticle/' + this.pageSize + '/1').then(resp => {
      ...
      handleCurrentChange (page) {
        var _this = this
        this.$axios.get('/guanggaoarticle/' + this.pageSize + '/' + page).then(resp => {
          if (resp && resp.data.code === 200) {
            _this.articles = resp.data.result.content
            _this.total = resp.data.result.totalElements
          }
      ...
      deleteArticle (id) {
        this.$confirm('此操作将永久删除该文章, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            this.$axios
              .delete('/admin/content/guanggaoarticle/' + id).then(resp => {
              if (resp && resp.data.code === 200) {
                this.loadArticles()
              }
            })
