  ...
  <div>
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false" type="warning" class="el-icon-close">取 消</el-button>
        <el-button type="primary" @click="onSubmit(selectedRole)" class="el-icon-check">确 定</el-button>
      </div>
    </el-dialog>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="primary"
              size="mini"
              @click="editRole(scope.row)" class="el-icon-edit">
              编辑
            </el-button>
            <el-button
    ...
    methods: {
      remove (row) {
        this.$confirm(`此操作将永久移除${row.nameZh}是否继续?`, '提示', {
          ...
          const rowName = row.nameZh
          this.$axios.delete('/admin/role/deleteRole/' + row.id).then(resp => {
            if (resp && resp.data.code === 200) {
              const indexs = this.roles.findIndex(row => {
                if (row.id === resp.data.result) {
                  return true
                }
              })
              this.roles.splice(indexs, 1)
              this.$notify.success({
                title: '移除成功！',
